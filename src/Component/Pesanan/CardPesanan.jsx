  ...
  const ModalReview = ({ produk_id, currentUserId }) => {
    ...
    return (
      <Modal show={modalShow} onHide={() => setModalShow(false)} size="lg" aria-labelledby="contained-modal-title-vcenter" centered>
        ...
        <Modal.Body>
          <CommentReview getPesanan={getPesanan} dataItem={dataItem} product_id={produk_id} setModalShow={setModalShow} currentUserId={currentUserId} />
        </Modal.Body>
        <Modal.Footer>
                          ...
                          <>
                            ...
                            <Card.Body className="col-lg-8">
                              ...
                              {(item.status === '1' || item.status === '2') && !item.review ? (
                                ...
                                <>
                                  <button className="btn btn-warning mt-3" disabled style={{ marginLeft: 5 }}>
                                    Tracking
                                  </button>
                                </>
                              )}

                              {(item.status === '1' || item.status === '2') && !item.review && item.status === '2' ? (
                              ...
                              {item.status === '1' ? (
                                <button className="btn btn-primary mt-3" style={{ marginLeft: 5 }} onClick={() => handleDiterima(item.id)}>
                                  Diterima
                                </button>
                              ) : (
                                <button className="btn btn-primary mt-3" style={{ marginLeft: 5 }} disabled>
                                  Diterima
                                </button>
                              )}
                            </Card.Body>
